...
import type { ModelMetric, PredictionResponse } from './types';
...
const MOCK_MODELS: ModelMetric[] = [
  { id: 'wav2vec2-base', name: 'Wav2Vec2.0 Base', accuracy: 0.89, f1_score: 0.88, status: 'active' },
  { id: 'hubert-large', name: 'HuBERT Large', accuracy: 0.92, f1_score: 0.91, status: 'active' },
  { id: 'whisper-emo', name: 'Whisper Emotion X', accuracy: 0.94, f1_score: 0.93, status: 'active' },
];
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in slide-in-from-bottom-8 fade-in duration-700 pb-12">
                <div className="lg:col-span-1 bg-gradient-to-br from-surface/80 to-surface/40 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-2xl relative overflow-hidden group">
                  ...
                  <p className="text-xs text-text-secondary mt-6 border-t border-white/10 pt-4">
                    Based on <strong>{MOCK_MODELS.find(m => m.id === selectedModel)?.name}</strong> model
                  </p>
                </div>
